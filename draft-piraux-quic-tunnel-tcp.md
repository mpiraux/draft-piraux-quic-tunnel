...
called the stream mode. It takes advantage of the QUIC streams to
effciently transport TCP
...
The TCP Connect TLV is used to requested the establishment of a TCP connection
...
connection part of the tunneling session using the TCP Resume TLV.
The Error TLV is
used to indicate any error that occurred during the establishment of a
TCP connection. Finally, the End TLV marks the end
...
In {{tlvs-in-stream}}, two examples of QUIC tunnel streams TLVs are
given. In the first one, the client opens Stream 0 and sends three TLVs. The
first one requests the concentrator to establish a new TCP connection. This
...
limited to do the peer that sent it.
